"""Execution nodes for the CoE guide agent LangGraph."""
...
async def planner_node(state: GuideAgentState) -> GuideAgentState:
    """Create or extend the task plan based on the raw request."""
...
async def knowledge_node(state: GuideAgentState) -> GuideAgentState:
    """Collect lightweight insights from local documentation."""
...
async def advisor_node(state: GuideAgentState) -> GuideAgentState:
    """Transform plans and insights into actionable recommendations."""
...
async def feedback_node(state: GuideAgentState) -> GuideAgentState:
    """Produce a final summary and roll the memory forward."""
...
def _load_doc_highlights() -> dict[str, List[str]]:
    """Read a few snippet lines from project documentation."""

    ko_snippets: List[str] = []
    en_snippets: List[str] = []
    root = Path(__file__).resolve().parents[3]
    candidates: Iterable[tuple[str, Path]] = (
        ("ko", root / "docs" / "guide_agent_brief.md"),
        ("ko", root / "AGENTS.md"),
        ("ko", root.parent / "CoE-Backend" / "AGENTS.md"),
        ("en", root / "README.md"),
    )
...
async def _gather_rag_insights(
    ...
    try:
        ...
        for entry in results:
            label = entry.metadata.get("file_path") or entry.metadata.get("document_type")
            prefix = f"[{label}] " if label else ""
            snippets.append(f"{prefix}{entry.content}")
